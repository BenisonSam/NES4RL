...
namespace nes
{
		...
		if (!(value & 0x80)) //if reset bit is NOT set
		{
			...
			if (m_writeCounter == 5)
			{
				...
				} else if (addr <= 0xbfff) //CHR Reg 0
				{
					...
					m_firstBankCHR = &m_cartridge.getVROM()[0x1000 *
															(m_tempRegister | (1 - m_modeCHR))]; //OR 1 if 8KB mode
					if (m_modeCHR == 0)
						m_secondBankCHR = m_firstBankCHR + 0x1000;
				} else if (addr <= 0xdfff)
				{
	...
	void MapperSxROM::writeCHR(Address addr, Byte value)
	{
		...
		else LOG(Info) << "Read-only CHR memory write attempt at " << std::hex << addr << std::endl;
	}
}
